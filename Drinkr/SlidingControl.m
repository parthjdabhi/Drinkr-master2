...
#import "SlidingControl.h"
#import <QuartzCore/QuartzCore.h>
#import <math.h>

@interface PDScrollView : UIScrollView
@end

@interface SlidingControl ()
...
- (id)initWithSectionImages:(NSArray*)sectionImages sectionSelectedImages:(NSArray*)sectionSelectedImages {
    ...
    if (self) {
        ...
        self.type = SlidingControlTypeImages;
    }
...
- (instancetype)initWithSectionImages:(NSArray *)sectionImages sectionSelectedImages:(NSArray *)sectionSelectedImages titlesForSections:(NSArray *)sectiontitles {
    ...
    if (self) {
        ...
        self.sectionSelectedImages = sectionSelectedImages;
		self.sectionTitles = sectiontitles;
        self.type = SlidingControlTypeTextImages;
    }
...
- (void)commonInit {
    self.scrollView = [[PDScrollView alloc] init];
    ...
    self.selectionStyle = SlidingControlSelectionStyleTextWidthStripe;
    self.selectionIndicatorLocation = SlidingControlSelectionIndicatorLocationUp;
    self.segmentWidthStyle = SlidingControlSegmentWidthStyleFixed;
...
- (void)setSelectionIndicatorLocation:(SlidingControlSelectionIndicatorLocation)selectionIndicatorLocation {
	_selectionIndicatorLocation = selectionIndicatorLocation;
	
	if (selectionIndicatorLocation == SlidingControlSelectionIndicatorLocationNone) {
...
- (void)setSegmentWidthStyle:(SlidingControlSegmentWidthStyle)segmentWidthStyle {
    // Force SlidingControlSegmentWidthStyleFixed when type is SlidingControlTypeImages.
    if (self.type == SlidingControlTypeImages) {
        _segmentWidthStyle = SlidingControlSegmentWidthStyleFixed;
    } else {
        _segmentWidthStyle = segmentWidthStyle;
    }
}

- (void)setBorderType:(SlidingControlBorderType)borderType {
...
- (CGSize)measureTitleAtIndex:(NSUInteger)index {
    ...
    if ([title isKindOfClass:[NSString class]] && !self.titleFormatter) {
        ...
        NSAttributedString *str = [[NSAttributedString alloc] initWithString:title attributes:self.titleFormatter];
        size = [str size];
...
- (NSAttributedString *)attributedTitleAtIndex:(NSUInteger)index {
    ...
    if ([title isKindOfClass:[NSAttributedString class]]) {
        ...
        return [[NSAttributedString alloc] initWithString:title attributes:self.titleFormatter];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == SlidingControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            BOOL locationUp = (self.selectionIndicatorLocation == SlidingControlSelectionIndicatorLocationUp);
            BOOL selectionStyleNotBox = (self.selectionStyle != SlidingControlSelectionStyleBox);
            ...
            CGRect rect;
            if (self.segmentWidthStyle == SlidingControlSegmentWidthStyleFixed) {
            ...
            } else if (self.segmentWidthStyle == SlidingControlSegmentWidthStyleDynamic) {
    ...
    } else if (self.type == SlidingControlTypeImages) {
        [self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
            CGFloat y = roundf(CGRectGetHeight(self.frame) - self.selectionIndicatorHeight) / 2 - imageHeight / 2 + ((self.selectionIndicatorLocation == SlidingControlSelectionIndicatorLocationUp) ? self.selectionIndicatorHeight : 0);
    ...
    } else if (self.type == SlidingControlTypeTextImages){
		[self.sectionImages enumerateObjectsUsingBlock:^(id iconImage, NSUInteger idx, BOOL *stop) {
            UIImage *icon = iconImage;
            CGFloat imageWidth = icon.size.width;
            CGFloat imageHeight = icon.size.height;
    ...
    if (self.selectedSegmentIndex != SlidingControlNoSegment) {
        if (self.selectionStyle == SlidingControlSelectionStyleArrow) {
...
- (void)addBackgroundAndBorderLayerWithRect:(CGRect)fullRect {
    ...
    // Border layer
    if (self.borderType & SlidingControlBorderTypeTop) {
...
- (void)setArrowFrame {
    ...
    if (self.selectionIndicatorLocation == SlidingControlSelectionIndicatorLocationDown) {
    ...
    if (self.selectionIndicatorLocation == SlidingControlSelectionIndicatorLocationUp) {
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionStyle == SlidingControlSelectionStyleArrow) {
        ...
        if (self.selectionStyle == SlidingControlSelectionStyleTextWidthStripe &&
            sectionWidth <= self.segmentWidth &&
            self.segmentWidthStyle != SlidingControlSegmentWidthStyleDynamic) {
            CGFloat widthToEndOfSelectedSegment = (self.segmentWidth * self.selectedSegmentIndex) + self.segmentWidth;
            CGFloat widthToStartOfSelectedIndex = (self.segmentWidth * self.selectedSegmentIndex);
            ...
            if (self.segmentWidthStyle == SlidingControlSegmentWidthStyleDynamic) {
...
- (void)updateSegmentsRects {
    ...
    if (self.type == SlidingControlTypeText && self.segmentWidthStyle == SlidingControlSegmentWidthStyleFixed) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == SlidingControlTypeText && self.segmentWidthStyle == SlidingControlSegmentWidthStyleDynamic) {
    ...
    } else if (self.type == SlidingControlTypeTextImages && self.segmentWidthStyle == SlidingControlSegmentWidthStyleFixed){
        //lets just use the title.. we will assume it is wider then images...
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            CGFloat stringWidth = [self measureTitleAtIndex:idx].width + self.segmentEdgeInset.left + self.segmentEdgeInset.right;
            self.segmentWidth = MAX(stringWidth, self.segmentWidth);
        }];
    } else if (self.type == SlidingControlTypeTextImages && SlidingControlSegmentWidthStyleDynamic) {
        NSMutableArray *mutableSegmentWidths = [NSMutableArray array];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGRectContainsPoint(enlargeRect, touchLocation)) {
        ...
        } else if (self.type == SlidingControlTypeTextImages || self.type == SlidingControlTypeText) {
...
- (void)setSelectedSegmentIndex:(NSUInteger)index animated:(BOOL)animated notify:(BOOL)notify {
    ...
    if (index == SlidingControlNoSegment) {
        ...
        if (animated) {
            ...
            if(self.selectionStyle == SlidingControlSelectionStyleArrow) {
                ...
                if ([self.selectionIndicatorStripLayer superlayer] == nil) {
                    ...
                    if (self.selectionStyle == SlidingControlSelectionStyleBox && [self.selectionIndicatorBoxLayer superlayer] == nil)
                        [self.scrollView.layer insertSublayer:self.selectionIndicatorBoxLayer atIndex:0];
